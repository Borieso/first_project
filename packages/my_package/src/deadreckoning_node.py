...
class DeadReckoningNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.sub_reset_odom = rospy.Subscriber(f"{self._vehicle_name}/reset_odom", Odometry, self.reset_coordinates)

        self.sub_calibrate = message_filters.Subscriber
    ...
    def reset_coordinates(self, message):
        # Reset the x, y, z coordinates and orientation quaternion with semaphore protection
        with self.semaphore:
            ...
            _, _, self.yaw = tr.euler_from_quaternion(self.q)
            self.loginfo(f"Coordinates reset to x={self.x}, y={self.y}, z={self.z}, yaw={self.yaw:.2f} rad")
